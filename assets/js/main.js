...
const initPdfModal = () => {
  ...
  modalContainer.innerHTML = `
    <div class="pdf-modal-overlay" id="pdf-modal">
      <div class="pdf-modal-content" id="pdf-modal-content">
        ...
        <p style="margin: -8px 0 8px; color: #8a99ad; font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; text-align: center;">Company Profile</p>
        <div class="pdf-preview-container">
          <div class="pdf-loading-spinner" id="pdf-loading-indicator">Loading cover page...</div>
  ...
  const openModal = async () => {
    ...
    try {
      ...
      if (window.location.protocol === "file:") {
        indicator.textContent = "เบราว์เซอร์บล็อกการพรีวิวไฟล์แบบโลคอล (CORS/file://) กรุณาอัพขึ้นเซิร์ฟเวอร์เพื่อดูพรีวิว (แต่ยังสามารถดาวน์โหลด PDF ได้จากปุ่มด้านล่าง)";
      } else {
        indicator.textContent = "Failed to load preview. You can still download the PDF below.";
      }
...
// Start Easter Egg Scan logic on load
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initHoverScan);
} else {
  initHoverScan();
}
